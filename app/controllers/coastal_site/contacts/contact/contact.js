...
angular.module('GRPApp')
    .controller('CoastalContactCtrl', function ($scope, $q, $filter, coastalSiteService, contactService, $state, $stateParams, $mdDialog, $timeout, regex) {
        'use strict';
        $scope.regex = regex;


        if ($stateParams.contactid === 'new') {
            coastalSiteService.RelatedContacts.create($stateParams.siteid).then(function (contact) {
        ...
        $scope.selectedItemChange = function (contact) {
            if (contact) {
                ...
                if (contact.attributes.GlobalID && found.length > 0) {
                    $timeout(function () {
                        $state.go('main.coastal_site.contacts.contact', {contactid: $scope.contact.attributes.GlobalID});
                    }, 500);
                }
            }
        };
